    	...
    	if(hat.info != null && !hat.info.hatName.equalsIgnoreCase("") && !hat.renderingParent.isPlayerSleeping() && hat.renderingParent.isEntityAlive() && (Hats.renderHats == 1 || Hats.renderHats == 13131) && hat.render)
    	{
    		...
    		if((Hats.renderInFirstPerson == 1 && firstPerson || !firstPerson) && !hat.renderingParent.isInvisible())
    		{
    			...
    			if(Hats.hasMorphMod && hat.parent instanceof EntityPlayer)
    			{
    				...
    				{
    					float prog = MathHelper.clamp_float((((morph.api.Api.morphProgress(player.username, true) * 80F + renderTick) / 80F) - 1F/8F) / (6F/8F), 0.0F, 1.0F);
    					EntityLivingBase prevMorph = morph.api.Api.getPrevMorphEntity(player.username, true);
	    		...
	    		for(int i = 0; i < passesNeeded; i++)
	    		{
	    			...
	    			if(Hats.hasMorphMod && hat.parent instanceof EntityPlayer)
	    			{
	    				...
	    				if(morph.api.Api.hasMorph(player.username, true) && morph.api.Api.morphProgress(player.username, true) < 1.0F)
	    				{
	    					float realProg = morph.api.Api.morphProgress(player.username, true);
