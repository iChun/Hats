	...
	implements ISlider
{
	...
	public void onGuiClosed() 
	{
		...
		{
			stream1.writeByte(0);
			PacketDispatcher.sendPacketToServer(new Packet131MapData((short)Hats.getNetId(), (short)8, bytes.toByteArray()));
		}
		catch(IOException e)
		{}
	}
	...
	public void updateScreen() 
	{
		if(updateOffers)
		{
			...
			try
			{
				stream.writeInt(ourHatsForTrade.size());
				for(String s : ourHatsForTrade)
				{
					stream.writeUTF(s);
				}
				stream.writeInt(ourItemsForTrade.size());
				for(ItemStack is : ourItemsForTrade)
				{
					Hats.writeNBTTagCompound(is.writeToNBT(new NBTTagCompound()), stream);
				}
				PacketDispatcher.sendPacketToServer(new Packet131MapData((short)Hats.getNetId(), (short)9, bytes.toByteArray()));
			}
			catch(IOException e)
			{}
    ...
    protected void keyTyped(char c, int i)
    {
    	...
    	if(chatBar.isFocused() && i == Keyboard.KEY_RETURN && !chatBar.getText().isEmpty())
    	{
    		...
    		try
    		{
    			stream1.writeUTF(mc.thePlayer.username + ": " + chatBar.getText());

    			PacketDispatcher.sendPacketToServer(new Packet131MapData((short)Hats.getNetId(), (short)7, bytes.toByteArray()));
    		}
    		catch(IOException e)
    		{}
    		
    		chatMessages.add(mc.thePlayer.username + ": " + chatBar.getText());
    		chatBar.setText("");
    	}
        if (i == 1)
        {
...
//        	y >= guiTop + 18 && y < guiTop + 18 + 37
        ...
        fontRenderer.drawString(StatCollector.translateToLocalFormatted("hats.trade.theirOfferings", new Object[] { trader }), (int)(guiLeft + 125), (int)(guiTop + 105), 0x2c2c2c, false);
        
        fontRenderer.drawString(selfReady ? StatCollector.translateToLocal("hats.trade.tradeReady") : StatCollector.translateToLocal("hats.trade.tradeNotReady"), (int)((guiLeft + 187) - (fontRenderer.getStringWidth(selfReady ? StatCollector.translateToLocal("hats.trade.tradeReady") : StatCollector.translateToLocal("hats.trade.tradeNotReady")) / 2)), (int)((guiTop + 83)), selfReady ? 0x81b63a : 0x790000, false); //0x790000
        
        fontRenderer.drawString(theirReady ? StatCollector.translateToLocal("hats.trade.tradeReady") : StatCollector.translateToLocal("hats.trade.tradeNotReady"), (int)(guiLeft + 187 - (fontRenderer.getStringWidth(theirReady ? StatCollector.translateToLocal("hats.trade.tradeReady") : StatCollector.translateToLocal("hats.trade.tradeNotReady")) / 2)), (int)(guiTop + 176), theirReady ? 0x517924 : 0x790000, false); //0x790000
        ...
        fontRenderer.drawString(hasItem ? (selfReady && theirReady ? (pointOfNoReturn ? (clickedMakeTrade ? StatCollector.translateToLocal("hats.trade.waitingForThem") : StatCollector.translateToLocal("hats.trade.waitingForYou")) : StatCollector.translateToLocal("hats.trade.bothReady")) : StatCollector.translateToLocal("hats.trade.waitingForReady") ) : StatCollector.translateToLocal("hats.trade.waitingForOffer"), (int)((guiLeft + 187) / scale - (fontRenderer.getStringWidth(hasItem ? (selfReady && theirReady ? (pointOfNoReturn ? (clickedMakeTrade ? StatCollector.translateToLocal("hats.trade.waitingForThem") : StatCollector.translateToLocal("hats.trade.waitingForYou")) : StatCollector.translateToLocal("hats.trade.bothReady")) : StatCollector.translateToLocal("hats.trade.waitingForReady") ) : StatCollector.translateToLocal("hats.trade.waitingForOffer")) / 2)), (int)((guiTop + ySize - 10) / scale), -16777216, false);
        GL11.glPopMatrix();
    	...
    	if(grabbedStack == null)
    	{
	    	...
	    	else if(btn.id == ID_MAKE_READY)
	    	{
	            ...
	            if(hasItem && !pointOfNoReturn)
	            {
	        		...
	        		try
	        		{
	        			stream1.writeBoolean(selfReady);
	        			PacketDispatcher.sendPacketToServer(new Packet131MapData((short)Hats.getNetId(), (short)10, bytes.toByteArray()));
	        		}
	        		catch(IOException e)
	        		{}
	            }
	    	...
	    	else if(btn.id == ID_MAKE_TRADE)
	    	{
        		...
        		{
        			stream1.writeByte(0);
        			PacketDispatcher.sendPacketToServer(new Packet131MapData((short)Hats.getNetId(), (short)11, bytes.toByteArray()));
        		}
        		catch(IOException e)
        		{}
	    	}
    	...
    	for(int i = 0; i < chatMessages.size(); i++)
    	{
    		...
    		for(int kk = 0; kk < list.size(); kk++)
    		{
    			...
    			if(msg.startsWith(StatCollector.translateToLocal("hats.trade.terminatePrefix")))
    			{
    				if(kk == 0)
    				{
    					fontRenderer.drawString(" " + (String)list.get(kk), (int)((guiLeft + 8) / scale), (int)((guiTop + 115 + (lines * 5)) / scale), 0x790000, false);
    				}
    				else
    				{
    					fontRenderer.drawString(" " + (String)list.get(kk), (int)((guiLeft + 8) / scale), (int)((guiTop + 115 + (lines * 5)) / scale), 0x790000, false);
    			...
    			else
    			{
	    			if(kk == 0)
	    			{
			    		...
			    		String prefix = "";
			    		if(line.startsWith(mc.thePlayer.username) && line.substring(mc.thePlayer.username.length()).startsWith(":"))
			    		{
			    			prefix = mc.thePlayer.username + ":";
			    		}
	    			...
	    			{
	    				fontRenderer.drawString(" " + (String)list.get(kk), (int)((guiLeft + 8) / scale), (int)((guiTop + 115 + (lines * 5)) / scale), 14737632, false);
	    			}
    			}
    	   		lines++;
    		}
