	...
	implements ISlider
{
    ...
    protected void actionPerformed(GuiButton btn)
    {
    	...
    	if(personalizing)
    	{
	    	...
	    	else if(btn.id == ID_SET_FP)
	    	{
	    		...
	    		btn.displayString = StatCollector.translateToLocal("hats.gui.firstPerson") + ": " + (Hats.config.renderInFirstPerson == 1 ? StatCollector.translateToLocal("gui.yes") : StatCollector.translateToLocal("gui.no"));
                Hats.config.save();
	    	}
	    	else if(btn.id == ID_SHOW_HATS)
	    	{
	    		Hats.config.renderHats = Hats.config.renderHats == 1 ? 0 : 1;
	    		btn.displayString = StatCollector.translateToLocal("hats.gui.showHats") + ": " + (Hats.config.renderHats == 1 ? StatCollector.translateToLocal("gui.yes") : StatCollector.translateToLocal("gui.no"));
                Hats.config.save();
	    	}
    ...
    public void exitAndUpdate()
    {
		...
		{
            Hats.proxy.tickHandlerClient.playerWornHats.put(player.getCommandSenderName(), new HatInfo(hat.hatName, colourR, colourG, colourB, alpha));
    ...
    public void updateButtonList()
    {
        ...
        if(!personalizing)
        {
	    	...
	    	else if(view == VIEW_COLOURIZER)
	    	{
	    		...
	    		for(int i = 0; i < 4; i++)
	    		{
	    			GuiButton btn = new GuiSlider(i == 3 ? 29 : 5 + i, width / 2 - 6, height / 2 - 78 + (22 * button), 88, i == 0 ? StatCollector.translateToLocal("item.fireworksCharge.red") + ": " : i == 1 ? StatCollector.translateToLocal("item.fireworksCharge.green") + ": " : i == 2 ? StatCollector.translateToLocal("item.fireworksCharge.blue") + ": " : StatCollector.translateToLocal("hats.gui.alpha") + ": ", "", 0, 255, i == 0 ? colourR : i == 1 ? colourG : i == 2 ? colourB : alpha, false, true, this);
	    			buttonList.add(btn);
        ...
        else
        {
        	...
        	buttonList.add(new GuiButton(ID_SET_FP, width / 2 - 6, height / 2 - 78 + 22, 88, 20, StatCollector.translateToLocal("hats.gui.firstPerson") + ": " + (Hats.config.renderInFirstPerson == 1 ? StatCollector.translateToLocal("gui.yes") : StatCollector.translateToLocal("gui.no"))));
        	buttonList.add(new GuiButton(ID_SHOW_HATS, width / 2 - 6, height / 2 - 78 + (22 * 2), 88, 20, StatCollector.translateToLocal("hats.gui.showHats") + ": " + (Hats.config.renderHats == 1 ? StatCollector.translateToLocal("gui.yes") : StatCollector.translateToLocal("gui.no"))));
        	...
        	if(Hats.config.playerHatsMode < 4)
        	{
        		buttonList.add(new GuiSlider(ID_MOB_SLIDER, width / 2 - 6, height / 2 - 78 + (22 * 3), 88, StatCollector.translateToLocal("hats.gui.randomobs") + ": ", "%", 0, 100, Hats.config.randomMobHat, false, true, this));
        	}
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
        ...
        drawString(fontRendererObj, StatCollector.translateToLocal("hats.gui.viewing") + ": " + currentDisplay, this.guiLeft + 1, this.guiTop - 9, 0xffffff);
...
//	        RenderHelper.disableStandardItemLighting();
            ...
            Minecraft.getMinecraft().getRenderManager().renderEntityWithPosYaw(hat, 0.0D, /*hat.parent == hat.renderingParent ? 0.0F : hat.parent.yOffset*/ 0.0D, 0.0D, hat.rotationYaw, 1.0F);
