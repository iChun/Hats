...
public class HatHandler
{
    ...
    public static void requestHat(String name, EntityPlayer player)
    {
        if(player != null)
        {
            Hats.channel.sendToPlayer(new PacketRequestHat(name), player);
        }
        else
        {
                ...
                if(readHatFromFile(file))
                {
                    if(isServer)
                    {
                        ...
                        if(queuedLists != null)
                        {
                            ...
                            {
                                EntityPlayer player1 = FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().getPlayerByUsername(name);
                                if(player1 != null)
                                {
        ...
        if(file != null)
        {
            ...
            try
            {
                ...
                while(fileSize > 0)
                {
                    ...
                    if(player != null)
                    {
                        Hats.channel.sendToPlayer(new PacketHatFragment(hatFullName, packetsToSend, packetCount, fileSize > 32000 ? 32000 : fileSize, fileBytes), player);
                    }
                    else
                    {
    ...
    public static void unlockHat(EntityPlayer player, String hat)
    {
        ...
        while(ite.hasNext())
        {
            ...
            if(name.equalsIgnoreCase(hat))
            {
                ...
                Hats.channel.sendToPlayer(new PacketString(0, name), player);
