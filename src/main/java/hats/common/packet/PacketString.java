        ...
        if(side.isServer())
        {
            switch(pingId)
            {
                case 0: //Received Trade request
                {
                    ...
                    EntityPlayerMP plyr = FMLCommonHandler.instance().getMinecraftServerInstance().getConfigurationManager().func_152612_a(plyr1);
                    ...
                    if(plyr != null && plyr.isEntityAlive() && plyr.getDistanceToEntity(player) < 16D && plyr.canEntityBeSeen(player) && plyr.dimension == player.dimension)
                    {
                        ...
                        if(tr == null || !tr.traderName.equalsIgnoreCase(player.getCommandSenderName()))
                        {
                            ...
                            PacketHandler.sendToPlayer(Hats.channels, new PacketString(1, player.getCommandSenderName()), plyr); //send player the trade req.
                        }
                    }
                    else
                    {
